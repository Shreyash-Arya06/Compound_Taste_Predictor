...
class Genetic:
    def __init__(self, mutation_threshold = 0.025, parent_threshold = 0.75, mutation_factor = 0.15, size_of_group = 5, max_generation = 10):
    ...
    def evaluateChromosome(self, chromosome):
        ...
        file.write(f'cost: {np.round(chromosome.genes, 8)}, fitness: {round(chromosome.fitness, 8)}\n')
        file.close()
    ...
    def run(self):
        ...
        print(f'Best fitness: {round(best_chromosome.fitness, 8)}, weights: {np.round(best_chromosome.genes, 8)}')
